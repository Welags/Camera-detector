...
if __name__ == '__main__':
  ...
  ColorActivate = Scroll('Filtre (0/1)', 0, 0, 1, 1, 'CTRL')
  ColorBar = Scroll('Teinte', 0, 120, 179, 1, 'CTRL') 
  ...
  while vidin.isOpened() :
    ...
    activer_couleur = int(ColorActivate.value())
    teinte = int(ColorBar.value())
    ...
    if activer_couleur == 1:
        hsv_frame = cv.cvtColor(frame2, cv.COLOR_BGR2HSV)
        seuil_bas = np.array([max(0, teinte - 15), 50, 50])
        seuil_haut = np.array([min(179, teinte + 15), 255, 255])
        masque_couleur = cv.inRange(hsv_frame, seuil_bas, seuil_haut)

        masque_final = cv.bitwise_and(img, masque_couleur)
    else:
        masque_final = img

    (cont,_) = cv.findContours(masque_final,cv.RETR_EXTERNAL,cv.CHAIN_APPROX_SIMPLE)
    ...
    if activer_couleur == 1:
        info_filtre = f"Filtre Couleur: Actif (Teinte: {teinte})"
    else:
        info_filtre = "Filtre Couleur: Inactif"
        
    cv.putText(t, info_filtre, (10, 90), cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
    ...
    cv.imshow('contour', t)
    cv.imshow('masque combine', masque_final) 
